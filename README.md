<h1 align="center">Hi there👋, I'm Monilkumar Patel</h1>

<h3 align="center">Aspiring Data Analyst | Machine Learning & Data Science Enthusiast </h3>

---

## 🌟 About Me
Pursuing Master's in Web and Data Science program @ University of Koblenz. Computer Engineering Graduate with strong interests in Data Analytics/Engineering, AI, and Machine Learning.

- 🛠️ Core Focus: **Building scalable ETL pipelines, optimizing databases, and crafting interactive dashboards using Python, SQL, and modern BI tools**
- 💡 Passionate about turning **data into actionable insights & data visualization for business intelligence**
  
---

## 🚀 Featured Projects

| Project | Tech/Tools|
|--------|------|
| https://github.com/monil37/retail-churn-campaign-roi | Python, Power BI, SQL, DAX |
| https://github.com/monil37/Netflix-Data-Analysis-Power-BI <br> https://github.com/monil37/Power-BI-Classic-Model-Project-| Power BI, DAX |
| https://github.com/monil37/SQL-Case-Studies | MySQL Portfolio |
| https://github.com/monil37/Bike-Sales-Analysis-Tableau | Tableau |
| https://github.com/monil37/Local-LLMs-using-Ollama <br> https://github.com/monil37/local-llm-ollama-langchain | Python, Ollama, Langchain, Agents & tools |
| https://github.com/monil37/Movie-Recommender-System | Jupyter Notebook, Python, Streamlit|

---

